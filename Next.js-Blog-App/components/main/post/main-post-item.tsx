...
async function getPublicImageUrl(postId: string, fileName?: string) {
  if (!fileName) return "/images/not-found.jpg"; // fallback

  const cookieStore = cookies();
  const supabase = createClient(cookieStore);
  const { data } = supabase.storage
    .from(process.env.NEXT_PUBLIC_SUPABASE_STORAGE_BUCKET_POSTS || "posts")
    .getPublicUrl(`${postId}/${fileName}`);

  return data?.publicUrl ?? "/images/not-found.jpg";
}
...
const MainPostItem: React.FC<MainPostItemProps> = async ({ post }) => {
  ...
  // Safely get date
  const displayDate = post.updated_at || post.created_at || null;
  const formattedDate = displayDate ? format(parseISO(displayDate), "dd/MM/yyyy") : "—";
